...
import { aiResponses, suggestedQuestions } from '@/features/learn/data/learning-data';
        ...
        {displayContent ? (
          <div className="space-y-4">
            {displayContent.split('\n\n').map((para, i) => (
              <p key={i} className="text-[13px] text-[#2D2D2D] leading-[1.8] font-serif">{para}</p>
            ))}
          </div>
        ) : (
      ...
      <div className="w-[360px] flex-shrink-0 flex flex-col bg-[#F5F0EB]">
        <div className="flex items-center gap-2.5 px-4 py-3 border-b-2 border-[#333333]/15 bg-white/40 flex-shrink-0">
          ...
          {isStandalone && onClose && (
            <button onClick={onClose} className="ml-auto w-7 h-7 rounded-full bg-white border-2 border-[#333333]/30 flex items-center justify-center hover:border-[#333333] transition-colors">
              <X size={12} />
            </button>
          )}
          {!isStandalone && onBack && (
            <button onClick={onBack} className="ml-auto text-[11px] text-[#5A5C58] hover:text-[#2D2D2D] transition-colors font-medium cursor-pointer">
              ← Quay lại
            </button>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          {messages.map((msg) => (
            <AnimatePresence key={msg.id}>
              ...
              >
                {msg.role === 'ai' && (
                  <div className="w-7 h-7 rounded-xl bg-[#EEF2FF] border border-[#A5B4FC] flex items-center justify-center text-sm flex-shrink-0 mt-0.5">AI</div>
                )}
          ...
          <AnimatePresence>
            {showSuggested && (
              <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex flex-col gap-1.5 pt-1">
                {SUGGESTED.map((q) => (
                  <button key={q} onClick={() => sendMessage(q)} className="text-left px-3 py-2 rounded-xl bg-[#EEF2FF] border border-[#A5B4FC] text-[12px] text-[#4338CA] font-medium hover:bg-[#E0E7FF] transition-colors">
                    {q}
                  </button>
                ))}
              </motion.div>
