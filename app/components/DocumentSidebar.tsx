...
import { mindmapNodes, type LearningDocument } from '@/lib/learning-data';
...
export default function DocumentSidebar({ nodeId, onClose, onApply, onOpenDocument }: Props) {
      ...
      {nodeId && node ? (
        ...
        >
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[11px] font-bold text-[#5A5C58] uppercase tracking-wider">
                    Tài Liệu Học
                  </span>
                </div>
            ...
            <div className="mt-3 flex items-center gap-2">
              <div className="flex-1 h-1.5 bg-[#E5E5DF] rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min(100, (node.documents.length / 6) * 100)}%` }}
                />
              </div>
            ...
            {node.documents.map((doc, idx) => {
              ...
              return (
                <motion.div
                  ...
                  style={{
                    borderColor: isChecked ? '#6EE7B7' : '#E5E5DF',
                    boxShadow: isChecked ? '0 0 0 2px rgba(110,231,183,0.2)' : 'none',
                  }}
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      borderColor: isChecked ? '#059669' : '#CCCCCC',
                  ...
                  <div
                    ...
                    style={{ backgroundColor: docColor(doc.type).bg }}
                  >
                    {docIcon(doc.type)}
                  </div>
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      if (nodeId) onOpenDocument(doc.id, nodeId);
                    }}
                  >
          ...
          <div className="p-4 border-t-2 border-[#333333]/20 bg-white/40">
            {checked.size > 0 && (
              <p className="text-[12px] text-[#5A5C58] mb-2 text-center">
                {checked.size} tài liệu được chọn
              </p>
            )}
            <motion.button
              ...
              style={{
                backgroundColor: applied ? '#34D399' : checked.size > 0 ? '#4CD964' : '#E5E5DF',
                color: checked.size > 0 ? '#0F5132' : '#9CA3AF',
