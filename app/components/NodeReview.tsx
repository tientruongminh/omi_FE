...
import { quizQuestions, flashcards, essayQuestion, teachAIPrompt } from '@/lib/learning-data';
      ...
      <div className="flex flex-col items-center justify-center h-full gap-6 py-8">
        ...
        <p className="text-[#5A5C58] text-center">
          {score === quizQuestions.length
            ? 'Xuất sắc! Bạn trả lời đúng tất cả câu hỏi!'
            : score >= 2
            ? 'Khá tốt! Ôn lại phần chưa đúng nhé.'
            : 'Cần ôn luyện thêm. Đọc lại tài liệu và thử lại!'}
        </p>
        <button
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <span className="text-[12px] text-[#5A5C58] font-medium whitespace-nowrap">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <p className="text-[15px] font-bold text-[#2D2D2D] mb-4 leading-snug">
            {question.question}
          </p>
          ...
          <div className="grid grid-cols-1 gap-2.5 mb-4">
            {question.options.map((opt) => {
              ...
              if (selected) {
                if (opt.correct) {
                  bg = 'bg-[#D1FAE5] border-[#34D399]';
                  text = 'text-[#065F46]';
                } else if (opt.label === selected && !opt.correct) {
                  bg = 'bg-[#FEE2E2] border-[#FCA5A5]';
                  text = 'text-[#991B1B]';
                } else {
                  bg = 'bg-white border-[#E5E5DF] opacity-50';
                }
              }
              return (
                <button
                ...
                >
                  <span className={`w-7 h-7 rounded-full flex items-center justify-center text-[12px] font-bold border-2 flex-shrink-0 ${
                    selected && opt.correct
                      ? 'bg-[#34D399] border-[#059669] text-white'
                      : selected && opt.label === selected && !opt.correct
                      ? 'bg-[#FCA5A5] border-[#F87171] text-white'
                      : 'bg-[#F1F1EC] border-[#CCCCCC] text-[#5A5C58]'
                  }`}>
                    {opt.label}
                  </span>
  ...
  return (
    <div className="flex flex-col items-center h-full gap-5">
      ...
      <div className="text-[13px] text-[#5A5C58] font-medium">
        {idx + 1}/{cards.length}
        {remembered.size > 0 && (
          <span className="ml-2 text-[#34D399]">✓ {remembered.size} đã nhớ</span>
        )}
      </div>
      ...
      >
        ...
        >
          {/* Front */}
          <div
            className="absolute inset-0 flex flex-col items-center justify-center rounded-2xl border-2 border-[#A5B4FC] bg-[#EEF2FF] p-6"
            style={{ backfaceVisibility: 'hidden' }}
          >
            <div className="text-[11px] font-bold text-[#818CF8] uppercase tracking-wider mb-3">
              Câu hỏi
            </div>
            <p className="text-[17px] font-bold text-[#2D2D2D] text-center leading-snug">
              {card.front}
            </p>
          ...
          {/* Back */}
          <div
            className="absolute inset-0 flex flex-col items-center justify-center rounded-2xl border-2 border-[#6EE7B7] bg-[#D1FAE5] p-6"
            style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}
          >
            <div className="text-[11px] font-bold text-[#059669] uppercase tracking-wider mb-3">
              Đáp án
            </div>
            <p className="text-[14px] font-medium text-[#065F46] text-center leading-relaxed">
              {card.back}
            </p>
          </div>
        </motion.div>
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      ...
      <textarea
        ...
        className="flex-1 w-full p-4 rounded-xl border-2 border-[#E5E5DF] bg-white text-[13px] text-[#2D2D2D] placeholder-[#9CA3AF] resize-none focus:outline-none focus:border-[#6B2D3E] transition-colors min-h-[180px]"
      />
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      ...
      <textarea
        ...
        className="flex-1 w-full p-4 rounded-xl border-2 border-[#E5E5DF] bg-white text-[13px] text-[#2D2D2D] placeholder-[#9CA3AF] resize-none focus:outline-none focus:border-[#6B2D3E] transition-colors min-h-[150px]"
      />
...
export default function NodeReview({ onBack, title, onClose, standalone }: Props) {
    ...
    <div className="flex flex-col h-full bg-[#F5F0EB]">
      ...
      {standalone && (
        <div className="flex items-center gap-3 px-5 py-4 border-b-2 border-[#333333]/20 bg-white/40 flex-shrink-0">
          <div className="w-8 h-8 rounded-lg bg-[#FEE2E2] flex items-center justify-center text-base">
            📋
          </div>
